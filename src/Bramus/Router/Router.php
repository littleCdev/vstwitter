...
class Router
{
    ...
    /**
     * @var function The function to be executed when an exception occours in a route
     */
    private $_exceptionHandler = null;

    /**
     * Store a function that is called it an exception occours in a route
     */
    public function exceptionHandler($fn){
        $this->_exceptionHandler = $fn;
        //$this->_exceptionHandler("asd");
    }
     ...
     * @param object|callable $fn      The handling function to be executed
     */
    public function match($methods, $pattern, $before, $fn)
    {
        $pattern = $this->baseRoute . '/' . trim($pattern, '/');
        $pattern = $this->baseRoute ? rtrim($pattern, '/') : $pattern;
     ...
     * @param object|callable $fn      The handling function to be executed
     */
    public function all($pattern, $before, $fn)
    {
        $this->match('GET|POST|PUT|DELETE|OPTIONS|PATCH|HEAD', $pattern, $before, $fn);
    }
     ...
     *
     * @param string          $pattern A route pattern such as /about/system
     * @param object|callable $before  The function to be executed before
     * @param object|callable $fn      The handling function to be executed
     */
    public function get($pattern, $before, $fn)
    {
        $this->match('GET', $pattern,$before, $fn);
    }
     ...
     * @param object|callable $fn      The handling function to be executed
     */
    public function post($pattern, $before, $fn)
    {
        $this->match('POST', $pattern, $before, $fn);
    }
     ...
     * @param object|callable $fn      The handling function to be executed
     */
    public function patch($pattern, $before, $fn)
    {
        $this->match('PATCH', $pattern, $before, $fn);
    }
     ...
     * @param object|callable $fn      The handling function to be executed
     */
    public function delete($pattern, $before, $fn)
    {
        $this->match('DELETE', $pattern, $before, $fn);
    }
     ...
     * @param object|callable $fn      The handling function to be executed
     */
    public function put($pattern, $before, $fn)
    {
        $this->match('PUT', $pattern, $before, $fn);
    }
     ...
     * @param object|callable $fn      The handling function to be executed
     */
    public function options($pattern, $before, $fn)
    {
        $this->match('OPTIONS', $pattern, $before, $fn);
    }
        ...
        foreach ($routes as $route) {
            ...
            if ($is_match) {
                ...
                if(isset($route['before'])){
                    if(gettype($route['before']) == 'array'){
                        foreach($route['before'] as $fn){
                            $ret = $this->invoke($fn, $params);
                            if($ret === false)
                                break;
                        }
                    }else{
                        $ret = $this->invoke($route['before'], $params);
                    }
                }
                if($ret !== false){
                    //var_dump($route);
        ...
        if (is_callable($fn)) {
            try{
                ...
                if($this->_exceptionHandler != null){
                    array_map($this->_exceptionHandler,[$e]);
                    return false;
                }else{
                    throw $e;
                }               
